const fs = require('fs');
var usuarios;
if (fs.existsSync('./users_levels.json')) {
	usuarios = require('./users_levels.json');
} else {
	fs.writeFile('./users_levels.json', '{}', (err) => {
		if (err) {
			console.log(err);
		}
		usuarios = require('./users_levels.json');
...
function verNivel(user) {
	...
	if (xp >= sigNivel) {
		...
		this.embed = new Discord.MessageEmbed()
			...
			.setDescription(
				`El usuario <@${user.id}>  ha ascendido al nivel ${nivel + 1}`
			)
			.setThumbnail(
...
function consultarXp(message) {
	...
	this.embed = new Discord.MessageEmbed()
		...
		.addFields(
			{
				name: `** Aca podras consultar tu nivel. **`,
				value: `${this.user} | TU NIVEL ES:`,
			},
