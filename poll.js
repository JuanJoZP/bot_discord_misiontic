...
client.on("message", msg => {
  if (msg.content == `${prefix}help poll`) {
    return msg.reply(
      `Este comando se usa asi: \`${prefix}poll titulo / pregunta / opcion1, opcion2, etc\``
    )
  }

  if (msg.content.startsWith(`${prefix}poll`)) return
  if (!msg.member.hasPermission("MANAGE_GUILD")) return msg.delete()

  const title = msg.split("/")[0].split(" ")[0]
  const question = msg.split("/")[1]
  const options = msg.split("/")[2].split(",")

  pollEmbed(msg, title, question, options)
})
